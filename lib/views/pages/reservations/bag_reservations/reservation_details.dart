...
class BagReservationDetails extends StatelessWidget {
  final Map<String, dynamic> reservation;

  const BagReservationDetails({super.key, required this.reservation});
                ...
                [
                            ...
                            Column(
                              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                                ...
                                children: [
                                  ...
                                  SizedBox(height: 10,),
                                  (reservation["reservation_data"]["description"]==null||reservation["reservation_data"]["description"]=="")?SizedBox():"Description".tr().text.xl.semiBold.white.make(),
                                  (reservation["reservation_data"]["description"]==null||reservation["reservation_data"]["description"]=="")?SizedBox():Text("${reservation["reservation_data"]["description"]}",style: TextStyle(color: Colors.white),).text.lg.semiBold.make().p8().glassMorphic().wFull(context),
                                  15.heightBox,
                                  "Details".text.xl2.bold.make(),
                                  SizedBox(height: 10,),
                                  Row(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      reservation["reservation_data"]["image"]==null?SizedBox().expand():Image.network("https://huopsapp.it/"+"${reservation["reservation_data"]["image"]}",).box.roundedSM.clip(Clip.antiAlias).make().glassMorphic().expand(),

                                      5.widthBox,
                                      Column(
                                        children: [
                                          "Price: ${reservation["reservation_data"]["price"]}".tr().text.xl.make().p8().glassMorphic().wFull(context),
                                          5.heightBox,
                                          "Size: ${reservation["reservation_data"]["bag_size"]}".tr().text.xl.make().p8().glassMorphic().wFull(context),
                                        ],
                                      ).expand(),
                                    ],
                                  ),
                  ...
                  reservation["reservation_data"]["status"] == "Cancelled"?SizedBox():reservation["reservation_data"]["status"] == "Rejected"&&reservation["reservation_data"]["reason"]!=null?reservation["reservation_data"]["reason"].toString().text.xl.make().p8().glassMorphic().wFull(context).px20():vm.isBusy?LoadingShimmer():Padding(
                    ...
                    child: CustomButton(
                      ...
                      onPressed:vm.cancelBagReservation,
                    ),
                  ),
